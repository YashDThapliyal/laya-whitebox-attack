...
- `laya-monitor/` — the checkpoint in `laya.load()`-compatible layout: `rl_agent_config.json` (includes the monitor question, fitted temperature, val threshold), tokenizer, encoder config. `model.safetensors` (1.7 GB) is gitignored; a copy was downloaded to this folder locally (and the Colab VM copy is gone once the session is stopped).
